...
DATABASE_FILE = os.path.join(DATABASE_DIR, "portfolio_analysis.db")
...
def get_db_connection():
    ...
    try:
        if not os.path.exists(DATABASE_DIR):
            ...
            print(f"[DATABASE] Created directory: {DATABASE_DIR}")
...
def save_to_database(df, table_name, if_exists='replace'):
    ...
    if df is None or df.empty:
        print(f"[WARNING] Cannot save empty DataFrame to table '{table_name}'")
        return False
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"[DATABASE] Successfully saved to table '{table_name}' ({len(df)} rows)")
        return True
    except sqlite3.Error as e:
        print(f"[ERROR] SQLite error while saving to '{table_name}': {e}")
        return False
    except Exception as e:
        print(f"[ERROR] Unexpected error while saving to database: {e}")
        return False
...
def fetch_historical_data(instrument_key, from_date, to_date):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 401:
                    print(f"[ERROR] Authentication failed. Please check ACCESS_TOKEN.")
                    return pd.DataFrame()
                elif response.status_code == 429:
                    print(f"[WARNING] Rate limit hit. Retrying in {RETRY_DELAY} seconds...")
                    time.sleep(RETRY_DELAY)
                else:
                    print(f"[ERROR] HTTP {response.status_code} for {instrument_key}: {e}")
                    if attempt == MAX_RETRIES - 1:
                        return pd.DataFrame()
            except requests.exceptions.RequestException as e:
                print(f"[ERROR] Request failed for {instrument_key} ({chunk_from} to {chunk_to}): {e}")
                if attempt < MAX_RETRIES - 1:
                    time.sleep(RETRY_DELAY)
                else:
                    return pd.DataFrame()
    ...
    if all_data:
        ...
        # Fix timezone handling - only localize if timezone-aware
...
def evaluate_combination(combination, all_stock_data, benchmark_metrics_overall):
    """
    Evaluates a single combination of stocks using 120-day lookback for optimization.
    Rebalances every 120 days with 60-day forward performance evaluation.
    ...
    # Fixed: Use START_DATE instead of hardcoded date, add 120 days for first lookback
    start_date_dt = datetime.strptime(START_DATE, '%Y-%m-%d')
    current_date = start_date_dt + timedelta(days=120)  # Start after first lookback period
    end_date_dt = datetime.strptime(END_DATE, '%Y-%m-%d')

    while current_date <= end_date_dt:
        rebalance_date = current_date
        # Changed to 120-day lookback for better drawdown prediction
        lookback_start_date = rebalance_date - timedelta(days=120)
        forward_end_date = rebalance_date + timedelta(days=59)

        # 1. Lookback & Optimization (120 days)
        ...
        if lookback_returns_df.shape[0] < 2 or lookback_returns_df.shape[1] != 5:
            current_date += timedelta(days=120)  # Changed: Move by 120 days if insufficient data
            continue

        optimal_weights = optimize_weights(lookback_returns_df)

        # 2. Forward Performance (60 days)
        ...
        current_date += timedelta(days=120)  # Changed: Rebalance every 120 days instead of 60
...
def analyze_portfolio():
    ...
    print(f"Strategy: 120-day lookback, 60-day forward window, Rebalance every 120 days")
    print(f"Optimization: HRP (Hierarchical Risk Parity) - MINIMIZE Drawdown FIRST, then Max Sharpe+Sortino")
    print(f"[DATABASE] Results will be saved to: {DATABASE_FILE}")
    ...
    # Save to database
    save_to_database(main_report_df, "rebalanced_portfolios")
    ...
    # Save to database
    save_to_database(yearly_report_df, "yearly_profits")

    print("\n--- Top 5 Portfolios ---")
    print(main_report_df.head(5).to_string())
    
    print(f"\n[SUCCESS] All results saved to database: {os.path.abspath(DATABASE_FILE)}")
